...
exports.NormalizeTournamentMode = NormalizeTournamentMode;
exports.GetTournamentFormat = GetTournamentFormat;
exports.GetCompetitorSlots = GetCompetitorSlots;
exports.CalculateRoundCount = CalculateRoundCount;
exports.GetQualificationCount = GetQualificationCount;
exports.IsTournamentPhaseType = IsTournamentPhaseType;
exports.BuildPrizeBands = BuildPrizeBands;
exports.BuildFormatFields = BuildFormatFields;
/** Normaliza dados novos e legados (`times`, party 1 + capacidade > 2). */
function NormalizeTournamentMode(tournament) {
    const raw = String(tournament?.Properties?.Mode || tournament?.mode || "").toLowerCase();
    if (raw === "solo" || raw === "ffa" || raw === "free-for-all")
        return "solo";
...
function GetCompetitorSlots(maxInvites, tournamentOrParty) {
...
function CalculateRoundCount(maxInvites, tournament, phaseType) {
...
function GetQualificationCount(tournament, isLastRound) {
    if (isLastRound)
        return 1;
    return Math.max(1, Math.floor(GetTournamentFormat(tournament).maxTeamsPerMatch / 2));
}
function IsTournamentPhaseType(phase, expected) {
 ...
 */
function BuildPrizeBands(totalGems, maxInvites, tournamentOrParty) {
    ...
    if (slots >= 2)
        bands.push({ position: 2, endPosition: 2, weight: 3, label: "Top 2" });
...
//# sourceMappingURL=TournamentRules.js.map