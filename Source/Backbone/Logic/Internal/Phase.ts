  ...
  for (const Match of Matches) {
    ...
    for (const [TeamId, TeamUsers] of Teams.entries()) {
      ...
      const Members = new Set<string>(
        TeamUsers.map((TeamUser) => String(TeamUser["@user-id"])).filter(Boolean)
      );
...
export async function CheckPhases(T: ITournament): Promise<void> {
  ...
  if (AllMatchesDone) {
    ...
    if (!T.NextPhaseStarted || T.NextPhaseStarted.getTime() > EarlyTarget.getTime()) {
      await Tournament.updateOne({ TournamentId: T.TournamentId }, { $set: { NextPhaseStarted: EarlyTarget } });
...
async function QualifyTop(TournamentId: string, PhaseId: number, GroupId: number, Count: number): Promise<void> {
  ...
  for (const MatchDoc of CompletedMatches) {
    ...
    for (const MatchUser of MatchDoc.users || []) {
      ...
      if (!Teams.has(TeamId)) Teams.set(TeamId, new Set<string>());
    ...
    for (const Members of Teams.values()) {
      ...
      if (TeamMap.has(TeamKey)) continue;
  ...
  for (const Team of TeamMap.values()) {
    for (const MemberId of Team.Members) CoveredUserIds.add(String(MemberId));
  }

  for (const User of Users) {
    if (CoveredUserIds.has(String(User.UserId))) continue;
    const Data = (User.Tournaments as any).get
      ? (User.Tournaments as any).get(TournamentId)
      : User.Tournaments[TournamentId];
    const Pos = Data?.UserPosition?.find((P: any) => P.phaseid === PhaseId && P.groupid === GroupId);
    if (!Pos) continue;
    const Members = new Set<string>([String(User.UserId)]);
...
export async function QualifyPhase(User: IBackboneUser, Tournament: ITournament) {
  ...
  const WinMembers = new Set<string>(WinIds);
  const LoseMembers = new Set<string>(LoseIds);
  ...
  const Claimed = await ClaimQualification(DbMatch.id);
  if (!Claimed) return;
    ...
    if (AllDone) {
      ...
      for (const FinalMatch of FinalRoundMatches) {
        ...
        for (const Members of Teams.values()) {
          ...
          const Data = LeaderUser
            ? (LeaderUser.Tournaments as any)?.get
              ? (LeaderUser.Tournaments as any).get(TournamentId)
              : (LeaderUser.Tournaments as any)?.[TournamentId]
            : null;
          const Pos = Data?.UserPosition?.find((P: any) => P.phaseid === PhaseId && P.groupid === GroupId);
          if (!Pos) continue;
      ...
      if (Sorted.length > 0) {
        ...
        if (TourWinners.length > 0) {
          await Tournament.updateOne(
            { TournamentId: Tournament.TournamentId, Status: { $ne: TournamentStatus.Finished } },
            {
              $addToSet: { Winners: { $each: TourWinners } },
              $set: { Status: TournamentStatus.Finished, "Properties.FinishedAt": new Date() },
            }
          );
        }
      }
