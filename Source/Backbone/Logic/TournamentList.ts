...
import { TournamentStatus } from "../Config";
...
export async function GetTournamentList(
  ...
  try {
    ...
    const active: any[] = [];
    const finished: any[] = [];
    for (const Tour of FoundTournaments as any[]) {
    ...
    for (let si = 0; si < ordered.length; si++) {
      ...
      if (id) typeById.set(id, si + 1); // 1..10
    }
    ...
    for (let i = 0; i < ordered.length; i++) {
      ...
      const hasWinners =
        Array.isArray(Tour.Winners) && Tour.Winners.length > 0;
      const isReallyFinished = hasWinners;
      const isReallyRunning =
        !isReallyFinished &&
        (Tour.Status === TournamentStatus.Running || now >= realStartMs);
      ...
      if (Tour.TournamentType !== typeForSlot) {
        Tournament.updateOne(
          { TournamentId: Tour.TournamentId },
          { $set: { TournamentType: typeForSlot } }
        ).catch(() => {});
      }
      ...
      let checkIn = false;
      if (DatabaseUser && (DatabaseUser as any).Tournaments) {
        const map = (DatabaseUser as any).Tournaments;
        const Info =
          typeof map.get === "function" ? map.get(tourId) : map[tourId];
        if (Info?.SignedUp) {
          inviteStatus = 1;
      ...
      Tournaments.push({
        ...
        data: {
          "tournament-data": {
            ...
            "prize-setting": [
              {
                reward: BuildPrizeDistribution(Tour.PrizePoolGems || 0, Tour.MaxInvites || 1, Tour).map((prize) => ({
    ...
    process.stdout.write(
      `[GetTournamentList] ${Tournaments.length} ids=${Tournaments.map((t) => t.id).join(",")} types=${Tournaments.map((t) => t.type).join(",")} times=${Tournaments.map((t) => t.tournamenttime).join(",")}\n`
    );
