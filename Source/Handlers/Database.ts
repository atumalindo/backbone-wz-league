...
import { BuildFormatFields, GetTournamentFormat, NormalizeTournamentMode } from "../Backbone/Logic/TournamentRules";
...
function getEmoteFriendlyName(emoteId: number): string {
  const emoteName = Object.keys(Emotes).find(
    (key) => (Emotes as any)[key] === emoteId
  );
  return emoteName || `${emoteId}`;
}

function emoteIdsToDisplay(ids: number[]): string {
  const parts = ids
    .map((id) => {
      if (id === -2) return `${EMOJI.punch_fire} ${EMOJI.punch}`;
      if (id === -3) return EMOJI.emotes_none;
      ...
      if (name.includes("punch")) return `${EMOJI.punch_fire} ${EMOJI.punch}`;
      if (name.includes("banana")) return `${EMOJI.banana} ${EMOJI.banana_gold}`;
...
function buildEmotesText(tournament: any): string {
  ...
  if (Array.isArray(raw)) {
    selected = raw.map((s: any) => String(s)).filter(Boolean);
  ...
  const disabledEmotes: number[] = tournament.Properties?.DisabledEmotes || [];
  ...
  const allIds = Object.values(Emotes as any).filter(
  ...
  if (disabledEmotes.length >= 3 && allIds.length > 0) {
    ...
    if (allowed.length === 0) return EMOJI.emotes_none;
  }
...
function getPhaseTypeName(phaseType: number): string {
  switch (phaseType) {
...
function getRoundEmoteNames(tournament: any, phaseIndex: number, roundIndex: number): string[] {
  ...
  if (Array.isArray(raw)) return raw.map((item: any) => String(item)).filter(Boolean);
  return raw ? String(raw).split(",").map((item) => item.trim()).filter(Boolean) : [];
}

function getPhaseRoundMaps(tournament: any, phase: any, phaseIndex: number): string[] {
  const phaseMaps = phase?.RoundMaps || phase?.MapsByRound;
  if (Array.isArray(phaseMaps) && phaseMaps.length) return phaseMaps.map(String);
  const allMaps = tournament.Properties?.RoundMaps || tournament.RoundMaps || [];
  if (phaseIndex === 0 && Array.isArray(allMaps) && allMaps.length) return allMaps.map(String);
  const maps = Array.isArray(phase?.Maps) ? phase.Maps.map(String) : [];
  return maps.length ? maps : ["Block Dash"];
}

function emoteNamesToDisplay(names: string[]): string {
  const parts = names
...
function buildPhaseRoundLines(tournament: any, phase: any, phaseIndex: number, isLastPhase = false): { lines: string[]; customEmotes: boolean } {
  ...
  const lines: string[] = [];
  let start = 0;
  const keyOf = (round: { map: string; emotes: string[] }) => customEmotes
    ? `${round.map}\u0000${JSON.stringify(round.emotes)}`
    : round.map;
  for (let index = 1; index <= rounds.length; index++) {
    if (index < rounds.length && keyOf(rounds[index]) === keyOf(rounds[start])) continue;
...
function buildPayload(tournament: any) {
  ...
  if (tournament.Phases && tournament.Phases.length > 0) {
    ...
    tournament.Phases.forEach((phase: any, index: number) => {
      ...
      for (const line of grouped.lines) phasesContent += `${EMOJI.region} ${line}\n`;
    });
...
export async function UpdateTournamentWebhook(
  tournament: any
): Promise<void> {
  const anyT = tournament as any;
  const messageId =
    anyT.WebhookMessageId || anyT.Properties?.WebhookMessageId;
  const base = getWebhookBase();
  ...
  if (!messageId) {
    console.error(
      `[webhook] Torneio ${anyT.TournamentId} sem WebhookMessageId — embed não pode ser atualizada`
    );
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        `[webhook] update failed: ${response.status} - ${errorText}`
      );
    } else {
      console.log(
        `[webhook] ✅ updated ${anyT.TournamentId} → ${anyT.CurrentInvites}/${anyT.MaxInvites} (msg ${messageId})`
      );
    }
  } catch (err) {
...
export async function CreateTournament(tournamentData: TournamentInput) {
  ...
  const input: any = tournamentData as any;
  const mode = NormalizeTournamentMode(input);
  const format = GetTournamentFormat({ ...input, Properties: { ...(input.Properties || {}), Mode: mode } });
  const formatFields = BuildFormatFields({ ...input, Properties: { ...(input.Properties || {}), Mode: mode } });
  ...
  const maxPlayers = (tournamentData as any).MaxInvites || 0;
  if ((tournamentData as any).Phases?.length) {
    const correctMaxTeams =
      partySize >= 2 ? Math.floor(maxPlayers / partySize) : maxPlayers;
    for (const phase of (tournamentData as any).Phases) {
  ...
  try {
    ...
    if (messageId) {
      ...
      console.log(
        `[webhook] created message ${messageId} for tournament ${saved.TournamentId}`
      );
    }
  } catch (err) {
...
export async function CreateSignedUpUser(Times: number, TournamentId: string) {
  ...
  const partySize = GetTournamentFormat(DBTour).playersPerTeam;
...
export async function OnPlayerSignedUp(
  TournamentId: string,
  playersAdded: number = 1
): Promise<void> {
  const updated = await Tournament.findOneAndUpdate(
    { TournamentId },
    { $inc: { CurrentInvites: playersAdded } },
    { new: true }
  );
  if (updated) {
    await UpdateTournamentWebhook(updated);
...
export async function GetSmallestAvailableTournamentId(): Promise<string> {
  const used = new Set(
    (await Tournament.find({}, { TournamentId: 1 }).lean()).map((row: any) => Number(row.TournamentId)).filter((id: number) => Number.isInteger(id) && id > 0)
  );
  let id = 1;
