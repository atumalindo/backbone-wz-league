...
import { UpdateTournamentWebhook } from "../../Handlers/Database";
import { ChargeTournamentEntry, RefundTournamentEntry } from "../../Backbone/Logic/TournamentEconomy";
...
async function tryUpdateWebhook(tournamentId: string): Promise<void> {
  try {
    // tenta string e number
    let tour =
      (await Tournament.findOne({ TournamentId: tournamentId })) ||
      (await Tournament.findOne({ TournamentId: Number(tournamentId) as any }));
    ...
    const msgId =
      anyTour.WebhookMessageId || anyTour.Properties?.WebhookMessageId;

    console.log(
      `[signup] update embed id=${tournamentId} invites=${tour.CurrentInvites}/${tour.MaxInvites} msgId=${msgId || "NONE"}`
    );

    if (!msgId) {
      console.error(
        `[signup] Sem WebhookMessageId no torneio ${tournamentId}. A embed do Discord não foi salva no /create. Crie um torneio NOVO.`
      );
      return;
    }

    await UpdateTournamentWebhook(tour);
  } catch (err: any) {
...
App.post(
  ...
  async (req, res) => {
    ...
    if (Now < SignupOpensAt) {
      console.log(
        `[signup] BLOCKED too early tournament=${TournamentId} now=${Now.toISOString()} opens=${SignupOpensAt.toISOString()} start=${StartTime.toISOString()}`
      );
      return res.status(200).json({
    ...
    const EntryCharge = await ChargeTournamentEntry(DatabaseUser.UserId, TournamentId, CheckTournament.EntryFee || 0, `signup:${TournamentId}:${DatabaseUser.UserId}`);
    if (!EntryCharge.ok) {
      return res.status(200).json({
    ...
    if (!DatabaseTournament) {
      await RefundTournamentEntry(DatabaseUser.UserId, TournamentId, CheckTournament.EntryFee || 0, `refund:signup:${TournamentId}:${DatabaseUser.UserId}`);
      return res.status(200).json({
        status: TournamentSignUpStatus.TournamentIsFull,
        inviteId: null,
        inviteStatus: TournamentUserStatus.Invited,
    ...
    try {
      ...
      await RefundTournamentEntry(DatabaseUser.UserId, TournamentId, CheckTournament.EntryFee || 0);
    ...
    console.log(
      `[signup] OK player=${DatabaseUser.UserId} tournament=${TournamentId} invites=${DatabaseTournament.CurrentInvites}`
    );

    setImmediate(() => {
      tryUpdateWebhook(TournamentId).catch((err) =>
        console.error("[signup] webhook background error:", err)
      );
    });
  }
);
